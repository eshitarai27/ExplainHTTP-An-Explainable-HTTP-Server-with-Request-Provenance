...
ExplainHTTP is the foundation for a future project, ChaosHTTP, which will
inject latency, packet loss, and failures into the request lifecycle. To make
that possible without ever touching this codebase, every stage of the
pipeline fires a named hook through :class:`HookManager`. ChaosHTTP (or any
other extension) simply registers callables against these hook names; by
default the hook manager has no registered callbacks and behaves as a no-op.
