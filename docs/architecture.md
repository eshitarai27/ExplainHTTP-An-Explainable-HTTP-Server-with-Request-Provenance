...
3. **Extensible without modification.** The hook interface
   (`core/hooks.py`) exists so a future project (ChaosHTTP) can inject
   latency, packet loss, and failures into the request lifecycle without
   editing a single line of this repository.
...
extension seam. ExplainHTTP itself registers zero hooks by default. A future
project can call `app.hooks.register("before_handler", inject_latency)` from
*outside this repository* and have it fire on every request, with no core
code changes -- which is exactly the shape ChaosHTTP will need to inject
latency, packet loss, and induced failures.
