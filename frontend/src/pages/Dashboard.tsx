    ...
    <div className="flex flex-col gap-8">
      <div>
        ...
        <p className="mt-1 text-sm text-ink-muted dark:text-ink-muted-dark">
          Live overview of the ExplainHTTP server at{" "}
          <span className="tabular">{api.baseUrl}</span>
        </p>
      </div>

      {error && (
        <div className="rounded-md border border-status-critical/30 bg-status-critical/10 px-4 py-3 text-sm text-status-critical">
          {error} Start it with <code className="tabular">python backend/server.py</code>.
        </div>
      )}
      ...
      <div className="grid grid-cols-2 gap-4 md:grid-cols-5">
        ...
        />
        <StatTile label="Uptime" value={metrics ? formatUptime(metrics.uptime_seconds) : "—"} />
      </div>

      <div>
        <div className="mb-3 flex items-center justify-between">
          <h2 className="text-sm font-medium text-ink-muted dark:text-ink-muted-dark">
            Recent requests
          </h2>
          <Link
            to="/traces"
              ...
              {traces.length === 0 && (
                <tr>
                  <td colSpan={5} className="px-4 py-6 text-center text-ink-faint">
                    No requests recorded yet. Try{" "}
                    <code className="tabular">curl {api.baseUrl}/hello/world</code>
                  </td>
                </tr>
              )}
              {traces.map((t) => (
                ...
                >
                  ...
                  <td className="px-4 py-2.5">
                    <Link
                      to={`/traces/${t.trace_id}/timeline`}
                      className="tabular text-xs text-series-1 hover:underline"
                    >
